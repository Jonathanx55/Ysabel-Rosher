      ...
      <div className="max-w-2xl mx-auto px-4 py-32 text-center">
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.5 }} className="flex flex-col items-center">
          ...
          <p className="text-black/70 font-light mb-8">
            Tu pedido ha sido registrado. Haz clic en el siguiente botón para enviar tu pedido por WhatsApp y {formData.paymentMethod === 'transfer' ? 'enviar el comprobante de transferencia' : 'coordinar la entrega'}.
          </p>
          <div className="flex flex-col gap-4 sm:flex-row justify-center w-full max-w-sm">
            <a href={whatsappLink} target="_blank" rel="noopener noreferrer" className="w-full bg-green-500 text-white px-8 py-4 rounded-none text-xs uppercase tracking-widest hover:bg-green-600 transition-colors duration-300 text-center font-bold">
              Enviar a WhatsApp
            </a>
            <Link to="/catalog" className="w-full bg-black text-white px-8 py-4 rounded-none text-xs uppercase tracking-widest hover:bg-gold-500 hover:text-black transition-colors duration-300 text-center">
              Volver al Catálogo
            </Link>
          </div>
        </motion.div>
